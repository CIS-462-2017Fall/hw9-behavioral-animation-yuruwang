...
double BehaviorController::KSeparation = 50000.0; 
...

{
	...
	{ 
		...
		m_force[2] = gMass * gVelKv * (m_vd - v);
		double delta_theta = m_thetad - m_state[ORI][1];
		
		ClampAngle(delta_theta);
	
		m_torque[1] = gInertia * ((-1.0) * gOriKv * m_state[AVEL][1] + gOriKp * delta_theta);  
...
//  This function sets derive vector to appropriate values after being called
{
	...
	vec3  p_0 = state[POS];
	vec3 theta_b = state[ORI];
	vec3 velocity_b = state[VEL];
	vec3 angular_b = state[AVEL];

	stateDot[2] = force / gMass;
	stateDot[3] = torque / gInertia;

	stateDot[1] = angular_b;

	stateDot[0] = vec3(velocity_b.Length() * cos(theta_b[1]), 0.0, velocity_b.Length() * sin(theta_b[1]));
	m_Vel0 = stateDot[0];
...
void BehaviorController::updateState(float deltaT, int integratorType)
{
	...
	if (integratorType == 0) {
		...
		vector<vec3> x_pDot;
		for (int i = 0; i < m_stateDotDim; ++i) {
			x_p[i] = m_state[i] + deltaT * m_stateDot[i];
		}
		computeDynamics(x_p, m_controlInput, x_pDot, deltaT);

		for (int i = 0; i < m_stateDotDim; ++i) {
			m_state[i] = m_state[i] + (m_stateDot[i] + x_pDot[i]) * deltaT / 2.0;
		}
